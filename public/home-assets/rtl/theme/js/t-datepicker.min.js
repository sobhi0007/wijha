...
; (function ($) {
    'use strict'; var Defaults = { autoClose: true, durationArrowTop: 200, numCalendar: 2, titleCheckIn: 'Check In', titleCheckOut: 'Check Out', titleToday: 'Today', titleDateRange: 'night', titleDateRanges: 'nights', titleDays: ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'], titleMonths: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'Septemper', 'October', 'November', "December"], titleMonthsLimitShow: 20, replaceTitleMonths: null, showDateTheme: null, iconArrowTop: true, iconDate: '&#x279C;', arrowPrev: '&#x276E;', arrowNext: '&#x276F;', toDayShowTitle: true, dateRangesShowTitle: true, dateRangesHover: true, toDayHighlighted: false, nextDayHighlighted: false, daysOfWeekHighlighted: [0, 6], formatDate: 'yyyy-mm-dd', dateCheckIn: null, dateCheckOut: null, startDate: null, endDate: null, limitPrevMonth: 0, limitNextMonth: 11, limitDateRanges: 31, showFullDateRanges: false, fnDataEvent: null, mergeDataEvent: false, dateDisabled: [], valiDation: false }; var update_options; $.fn.tDatePicker = function (pr_el, options) {
        ...
        function setThemeCheckDate(pr_title, pr_class, pr_data_utc, pr_input, pr_fm_input) {
            return '<div class="py-2 border rounded-lg shadow mx-1 t-dates t-date-' + pr_class + '">' +
                settings.iconDate + '<label class="t-date-info-title">' + pr_title + '</label>' +
                showThemeDate(pr_class, pr_data_utc) + '</div>' + '<input required type="hidden"  id="' + pr_class.replace("-", "") + '" class="t-input-' + pr_class + '"   value="' + pr_fm_input + '" name="' + pr_class.replace("-", "_") + '">'
        }
        function showThemeDate(pr_class, pr_data_utc) {
            if (pr_data_utc !== 0 && pr_data_utc !== null) {
                ...
                if (settings.showDateTheme === 'dd') { return '<span class="text-muted t-day-' + pr_class + '"> ' + check_num_10(d.getDate()) + '</span>' } else if (settings.showDateTheme === 'dd/mm' || settings.showDateTheme === 'dd-mm') { return '<span class=" text-muted t-day-' + pr_class + '"> ' + check_num_10(d.getDate()) + '</span>' + '<span class="text-muted t-month-' + pr_class + '"> ' + showMonths + ' </span>' } else { return '<div style="position: absolute;top: 10px;direction: rtl;right: 45px;"><span class="text-muted t-day-' + pr_class + '"> ' + check_num_10(d.getDate()) + '</span>' + '<span class="text-muted t-month-' + pr_class + '"> ' + showMonths + ' </span>' + '<span class="text-muted t-year-' + pr_class + '"> ' + check_num_10(d.getFullYear()) + '</span></div>' }
            } else { return ''; }
        }
        ...
        function getDateUTC(pr_in, pr_out) {
            var Array_In_Out = ['check-in', 'check-out']; Array_In_Out.forEach(function (e) {
                var label_title = settings.titleCheckIn; var getDay = pr_in; var Input = 't_start'
            ...
            var d_utc = new Date(dataUTC[0]); var d_utc_co = new Date(dataUTC[1]); var limitRange = Date.UTC(d_utc.getFullYear(), d_utc.getMonth(), d_utc.getDate() + settings.limitDateRanges); var limitRangeCO = Date.UTC(d_utc_co.getFullYear(), d_utc_co.getMonth(), d_utc_co.getDate() - settings.limitDateRanges); for (var i = 0; i < toDayElement.length; i++) {
                ...
                toDayElement[i].onclick = function (e) {
                    ...
                    if ($(this).parents('.t-check-in').hasClass('t-check-in') === true) {
                        ...
                        var d = new Date(dataUTC[1]); var limitdate = Date.UTC(d.getFullYear(), d.getMonth(), d.getDate() - settings.limitDateRanges); data_utc_in = get_utc; data_utc_out = dataUTC[1]; if (get_utc === settings.startDate && $(this).hasClass('t_start') === false && dataUTC[0] >= settings.startDate) { data_utc_out = dataUTC[0]; }
                    ...
                    // checkin date
                    var checkin = data_utc_in;
                    var checkin = new Date(checkin);
                    var year = checkin.getFullYear();
                    var month = ('0' + (checkin.getMonth() + 1)).slice(-2);
                    var day = ('0' + checkin.getDate()).slice(-2);
                    var checkin = year + '-' + month + '-' + day;


                    //checkout date
                    var checkout = data_utc_out;
                    var checkout = new Date(checkout);
                    var year = checkout.getFullYear();
                    var month = ('0' + (checkout.getMonth() + 1)).slice(-2);
                    var day = ('0' + checkout.getDate()).slice(-2);
                    var checkout = year + '-' + month + '-' + day;

                      
                    if(checkin != checkout ){
                       daysCounter(checkin, checkout)
                    }
                  
                    function    daysCounter(checkin, checkout){
                        ...
                        var days = Math.floor(diffInMillis / (1000 * 60 * 60 * 24));
                        
                       document.getElementById('totalDays').textContent = days;
                       var price =  document.getElementById('price').innerHTML

                       document.getElementById('total').textContent = days*price;
                       document.getElementById('grandTotal').textContent = days*price;
                    }
                    ...
                    toDayElement[i].onmouseover = function (e) {
                        ...
                        function checkNumNight(pr_el, pr_date_utc) {
                            ...
                            if ($(pr_el).parents('.t-check-in').hasClass('t-check-in') === true) {
                                var nd = new Date(pr_date_utc[1]); if (el_hover === settings.startDate) { if ($(pr_el).parents('.t-check-in').find('.t_end').length === 0 && $(pr_el).parents('.t-check-in').find('.t_start').length === 0) { nd = new Date(pr_date_utc[0]); } }
