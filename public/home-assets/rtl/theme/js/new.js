...
; (function ($) {
    'use strict'; var Defaults = { autoClose: true, durationArrowTop: 200, numCalendar: 2, titleCheckIn: 'Check In', titleCheckOut: 'Check Out', titleToday: 'Today', titleDateRange: 'night', titleDateRanges: 'nights', titleDays: ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'], titleMonths: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'Septemper', 'October', 'November', "December"], titleMonthsLimitShow: 3, replaceTitleMonths: null, showDateTheme: null, iconArrowTop: true, iconDate: '&#x279C;', arrowPrev: '&#x276E;', arrowNext: '&#x276F;', toDayShowTitle: true, dateRangesShowTitle: true, toDayHighlighted: false, nextDayHighlighted: false, daysOfWeekHighlighted: [0, 6], formatDate: 'yyyy-mm-dd', dateCheckIn: null, dateCheckOut: null, startDate: null, endDate: null, limitPrevMonth: 0, limitNextMonth: 11, limitDateRanges: 31, showFullDateRanges: false, fnDataEvent: null }; var update_options; $.fn.tDatePicker = function (pr_el, options) {
        ...
        if (options !== undefined) {
            update_options.numCalendar = update_options.numCalendar || 2; var num_Limit = update_options.limitDateRanges || 31; var num_month_Limit = update_options.limitNextMonth || 11; var limitEndDate = new Date(sd.getFullYear(), sd.getMonth() + update_options.numCalendar + num_month_Limit - 1, sd.getDate()); limitEndDate = convertDateUTC(convertFormatDf(limitEndDate))
            if (pr_el === 'update') {
                if (options.length === 2) { checkCI(options[0]); checkCO(options[1]); } else { checkCI(options); }
        ...
        function checkCI(pr_options) {
            if (pr_options === '') { update_options.dateCheckIn = null; update_options.dateCheckOut = null; return; }
        ...
        function checkCO(pr_options) {
            var op_CI = convertDateUTC(convertFormatDf(update_options.dateCheckIn)); if (pr_options === '') {
                update_options.dateCheckIn = op_CI; if (isNaN(op_CI)) { update_options.dateCheckIn = null; }
            ...
            if (CO > update_options.startDate) {
                if (update_options.dateCheckIn === undefined || update_options.dateCheckIn === null) { update_options.dateCheckOut = CO; if (update_options.startDate > CO_dateLimit) { update_options.dateCheckIn = update_options.startDate; } else { update_options.dateCheckIn = CO_dateLimit; } } else {
                    update_options.dateCheckOut = CO; if (update_options.startDate > CO_dateLimit) { update_options.dateCheckIn = op_CI; if (CO < op_CI) { update_options.dateCheckIn = update_options.startDate; } } else {
            ...
            } else { update_options.dateCheckIn = update_options.startDate; update_options.dateCheckOut = nextDay; }
        }
        var findValueCI = this.find('.t-input-check-in').val(); var findValueCO = this.find('.t-input-check-out').val(); if (typeof (pr_el) === 'string') {
        ...
        if (typeof (pr_el) === 'string') {
            var get_startDate = settings.startDate || settings.dateCheckIn; if (pr_el === 'getStartDate') { return new Date(get_startDate) }
        ...
        function fnParents(pr_el_parent, pr_el_class) {
            var i = 0; while (pr_el_parent.className.match(pr_el_class) === null) {
        ...
        var aDays = settings.titleDays; function setDayOfWeek() {
        ...
        numCalendar = numCalendar - 1; function AppendDaysInMonth(pr_num) {
            var i = 0; var setTr = ''; while (i < pr_num) { setTr = setTr + '<tr>' + '<td class="t-day">1</td>' + '<td class="t-day">2</td>' + '<td class="t-day">3</td>' + '<td class="t-day">4</td>' + '<td class="t-day">5</td>' + '<td class="t-day">6</td>' + '<td class="t-day">0</td>' + '</tr>'; i++; }
            return setTr;
        }
        ...
        function convertFormatDf(pr_date) {
            ...
            if (typeof (pr_date) === 'string' && pr_date !== 'null') {
                var yyyy_mm_dd; if (pr_date.indexOf('/') !== -1) { pr_date = pr_date.replace(/\//g, '-') }
        ...
        function getDateUTC(pr_in, pr_out) {
            var Array_In_Out = ['check-in', 'check-out']; Array_In_Out.forEach(function (e) {
                var label_title = settings.titleCheckIn; var getDay = pr_in; var Input = 't-start'
                if (pr_in !== null) { label_title = ''; }
                if (e === 'check-out') {
                    label_title = settings.titleCheckOut; getDay = pr_out; if (pr_in === pr_out) { getDay = null; } else if (pr_out !== null) { label_title = ''; }
                    Input = 't-end'
                }
        ...
        var dataUTC = getDateUTC(settings.dateCheckIn, settings.dateCheckOut); function setDaysInMonth(pr_el, pr_data_utc) {
            var tswitch = pr_el.find('.t-month'); if (numCalendar >= 0) {
                for (var i_num = 0; i_num <= numCalendar; i_num++) {
                    ...
                    while (Date.UTC(date.getFullYear(), (date.getMonth())) === nextDate) {
                        days.push(date.getDay()); dataDays.push(date.getDate()); dataUTCDate.push(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate()))
        ...
        function getStyleDays(pr_el, pr_data_utc) {
            ...
            var d_utc = new Date(dataUTC[0]); var d_utc_co = new Date(dataUTC[1]); var limitRange = Date.UTC(d_utc.getFullYear(), d_utc.getMonth(), d_utc.getDate() + settings.limitDateRanges); var limitRangeCO = Date.UTC(d_utc_co.getFullYear(), d_utc_co.getMonth(), d_utc_co.getDate() - settings.limitDateRanges); for (var i = 0; i < toDayElement.length; i++) {
                var dayselect = toDayElement[i].getAttribute('data-t-date'); if (Number(dayselect) < settings.startDate) { toDayElement[i].className = 't-disabled'; }
                if (pr_el.hasClass('t-check-out') === true) {
                    ...
                    if (Number(dayselect) === dataUTC[0]) {
                        var Arrow = pr_el.parent().find('.t-arrow'); Arrow[0].className = Arrow[0].className.replace(' t-disabled', '') + ' t-disabled'
                    ...
                    if ($(pr_el).find('[data-t-date="' + co_n + '"]')[0] !== undefined) {
                        var Arrow = pr_el.parent().find('.t-arrow'); Arrow[Arrow.length - 1].className = Arrow[Arrow.length - 1].className.replace(' t-disabled', '') + ' t-disabled'
                ...
                toDayElement[i].onclick = function (e) {
                    e.stopPropagation(); if ($(this).hasClass('t-disabled') === true) { return; }
                ...
                if ('ontouchstart' in window === false) {
                    toDayElement[i].onmouseover = function (e) {
                        ...
                        function checkNumNight(pr_el, pr_date_utc) {
                            var el_hover = Number($(pr_el).attr('data-t-date')); var numDay = 0; if (e.target.className.indexOf('t-hover-day-content') !== -1 || e.target.className.indexOf('t-disabled') !== -1) { return; }
                            if ($(pr_el).parents('.t-check-in').hasClass('t-check-in') === true) {
                                var nd = new Date(pr_date_utc[1]); if (el_hover === settings.startDate) { if ($(pr_el).parents('.t-check-in').find('.t-end').length === 0 && $(pr_el).parents('.t-check-in').find('.t-start').length === 0) { nd = new Date(pr_date_utc[0]); } }
                                ...
                                while (el_hover < nd_1) {
                                    nd_1 = Date.UTC(nd.getFullYear(), nd.getMonth(), nd.getDate() - numDay); var t_this = $(pr_el).parents('.t-check-in').find('[data-t-date="' + nd_1 + '"]')[0]; if (t_this != undefined) { t_this.className = t_this.className.replace(' t-range-limit', '') + ' t-range-limit'; }
                                    numDay++; if (numDay > 1000) { return; }
                                }
                            }
                            if ($(pr_el).parents('.t-check-out').hasClass('t-check-out') === true) {
                                var nd = new Date(pr_date_utc[0]); var nd_1 = Date.UTC(nd.getFullYear(), nd.getMonth(), nd.getDate()); var limitday = Date.UTC(nd.getFullYear(), nd.getMonth(), nd.getDate() + settings.limitDateRanges); if (el_hover >= limitday) { return; }
                                while (el_hover != nd_1) {
                                    nd_1 = Date.UTC(nd.getFullYear(), nd.getMonth(), nd.getDate() + numDay); var t_this = $(pr_el).parents('.t-check-out').find('[data-t-date="' + nd_1 + '"]')[0]; if (t_this != undefined) { t_this.className = t_this.className.replace(' t-range-limit', '') + ' t-range-limit'; }
                        ...
                        if (settings.dateRangesShowTitle === true) {
                            var num_full = 1; if (settings.showFullDateRanges === true) { num_full = 0; settings.titleDateRange = settings.titleDateRanges; }
                        ...
                        this.onmouseout = function () {
                            ...
                            if (document.getElementsByClassName('t-range-limit').length != 0) {
                                var a = document.getElementsByClassName('t-range-limit'); a = [].slice.call(a)
                            ...
                            if (fnParents(this, 't-datepicker').getElementsByClassName('t-date-title').length != 0) {
                                var elem = fnParents(this, 't-datepicker').getElementsByClassName('t-date-title'); elem = [].slice.call(elem)
            ...
            if (settings.fnDataEvent != undefined && 'ontouchstart' in window === false) {
                for (var cl = 0; cl < settings.numCalendar; cl++) {
                    var t = new Date(pr_data_utc).getMonth(); var m = t + 1 + cl; if (m === 13) { m = m - 12 }
                    var gMonth = m; var gYear = new Date(pr_data_utc).getFullYear(); for (var i = 0; i < toDayElement.length; i++) {
        ...
        function clickEvent(pr_el, pr_data_utc) {
            var tArrow = pr_el.find('.t-arrow'); var df_toDay = new Date(settings.startDate); var end_Date = new Date(convertFormatDf(settings.endDate))
            ...
            var y = newDate.getFullYear(); var m = newDate.getMonth(); var d = newDate.getDate(); if (tArrow.length != 0) {
            ...
            var Arrow_2 = pr_el.find('.t-arrow'); if (Date.UTC(y, m + numCalendar) != limitNextMonth && Date.UTC(y, m) === limitPrevMonth) {
        ...
        function getTableCalendar(pr_el, pr_date_utc) {
            if ($(pr_el).find('.t-datepicker-day').length !== 0) {
                $('.t-datepicker-day').remove(); $('.t-arrow-top').css({ 'display': 'none' })
                $('html').removeClass('t-datepicker-open')
            } else {
                if ($(pr_el).parents('.t-datepicker').hasClass('t-datepicker').length !== 0) { $('.t-datepicker-day').remove(); $('.t-arrow-top').css({ 'display': 'none' }) }
                if (settings.iconArrowTop === true) {
                    var this_el_arrow = $(pr_el).parents('.t-datepicker'); if (this_el_arrow.find('.t-arrow-top').length === 0) { this_el_arrow.append('<span class="t-arrow-top"></span>') }
        ...
        var pr_callback = ''; function callEventClick(pr_callback, pr_date_utc) {
    ...
    'use strict'; var Defaults = { autoClose: true, durationArrowTop: 200, numCalendar: 2, titleCheckIn: 'Check In', titleCheckOut: 'Check Out', titleToday: 'Today', titleDateRange: 'night', titleDateRanges: 'nights', titleDays: ['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'], titleMonths: ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'Septemper', 'October', 'November', "December"], titleMonthsLimitShow: 3, replaceTitleMonths: null, showDateTheme: null, iconArrowTop: true, iconDate: '&#x279C;', arrowPrev: '&#x276E;', arrowNext: '&#x276F;', toDayShowTitle: true, dateRangesShowTitle: true, dateRangesHover: true, toDayHighlighted: false, nextDayHighlighted: false, daysOfWeekHighlighted: [0, 6], formatDate: 'yyyy-mm-dd', dateCheckIn: null, dateCheckOut: null, startDate: null, endDate: null, limitPrevMonth: 0, limitNextMonth: 11, limitDateRanges: 31, showFullDateRanges: false, fnDataEvent: null, mergeDataEvent: false, dateDisabled: [], valiDation: false }; var update_options; $.fn.tDatePicker = function (pr_el, options) {
        ...
        function getDateUTC(pr_in, pr_out) {
            var Array_In_Out = ['check-in', 'check-out']; Array_In_Out.forEach(function (e) {
                ...
                getDay = convertFormatDf(getDay)
                var formatDate = showValueInput(getDay); var required = ''; if (settings.valiDation === true && formatDate === null) {
                    formatDate = ''
                    required = 'required'
                }
        ...
        function getStyleDays(pr_el, pr_data_utc) {
            ...
            if (settings.dateDisabled != undefined && 'ontouchstart' in window === false) {
                var dataDis = settings.dateDisabled; for (var cl = 0; cl < settings.numCalendar; cl++) {
                    var t = new Date(pr_data_utc).getMonth(); var gMonth = t + 1 + cl; var gYear = new Date(pr_data_utc).getFullYear(); if (gMonth === 13) { gMonth = gMonth - 12; gYear = gYear + 1; }
            ...
            if (settings.fnDataEvent != undefined && 'ontouchstart' in window === false) {
                var data = settings.fnDataEvent; var mergeData = {}; for (var cl = 0; cl < settings.numCalendar; cl++) {
                    var t = new Date(pr_data_utc).getMonth(); var gMonth = t + 1 + cl; var gYear = new Date(pr_data_utc).getFullYear(); if (gMonth === 13) { gMonth = gMonth - 12; gYear = gYear + 1 }
        ...
        function clickEvent(pr_el, pr_data_utc) {
            ...
            var limitPrevMonth = Date.UTC(df_toDay.getFullYear(), df_toDay.getMonth() - settings.limitPrevMonth); var limitNextMonth = Date.UTC(end_Date.getFullYear(), end_Date.getMonth()) || Date.UTC(df_toDay.getFullYear(), df_toDay.getMonth() + numCalendar + settings.limitNextMonth); var newDate = new Date(pr_data_utc)
            var y = newDate.getFullYear(); var m = newDate.getMonth(); var d = newDate.getDate(); if (tArrow.length != 0) {
                for (var i = 1; i < tArrow.length - 1; i++) { tArrow[i].innerHTML = ''; tArrow[i].className = tArrow[i].className.replace(' t-disabled', '') + ' t-disabled'; }
                tArrow[0].onclick = function (e) {
                    e.stopPropagation()
                    if (Date.UTC(y, m) > limitPrevMonth) { m = m - 1; setDaysInMonth(pr_el, Date.UTC(y, m)) }
                }
