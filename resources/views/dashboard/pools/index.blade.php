...
@section('title', __('lang.pools'))
@section('pools_active', 'active bg-light')
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h2 class="h5 page-title">{{ __('lang.pools') }}</h2>

                @if (permission(['add_pools']))
                    <div class="page-title-right">
                        <a href="{{ route('admin.pools.create') }}" data-title="{{ __('lang.add_new_pool') }}" id="add_btn"
                        ...
                        @if (count($data['data']) > 0)
                            @foreach ($data['data'] as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            <button class="btn btn-primary dropdown-toggle btn-sm" pool="button"
                                                data-toggle="dropdown" aria-expanded="false">
                                                {{ __('lang.actions') }} <i class="mdi mdi-chevron-down"></i>
                                            </button>
                                            <div class="dropdown-menu">

                                                {{-- @if (permission(['show_pools']))
                                                    <a href="{{ route('admin.pools.show', ['pool' => $item]) }}"
                                                        class="dropdown-item displayClass"
                                                        data-title="{{ __('lang.show_pool') }}" data-toggle="modal"
                                                ...
                                                @if (permission(['edit_pools']))
                                                    <a href="{{ route('admin.pools.edit', ['pool' => $item]) }}"
                                                        class="dropdown-item editClass"
                                                        data-title="{{ __('lang.edit_pool') }}" data-toggle="modal"
                                                        data-target="#mainModal">
                                                ...
                                                @if (permission(['delete_pools']))
                                                    <a class="dropdown-item deleteClass"
                                                        href="{{ route('admin.pools.destroy', ['pool' => $item]) }}"
                                                        data-title="{{ __('lang.delete_pool') }}" data-toggle="modal"
